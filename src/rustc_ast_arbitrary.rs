...
use rustc_ap_rustc_ast::{Defaultness, Expr, ExprKind, ItemKind, NodeId, Path, PathSegment, Ty, TyKind, Visibility, VisibilityKind, ast::{Attribute, Item}, ptr::P};
use rustc_ap_rustc_span::{BytePos, Span, Symbol, hygiene::SyntaxContext, symbol::Ident, with_default_session_globals};
use std::iter;
...
struct Context {next_id: u32}

fn attrs<'a, V>(_ctx: &mut Context, _u: &mut Unstructured<'a>) -> V
where V: iter::FromIterator<Attribute> {
   iter::empty().collect()
}
...
where T: ContextArbitrary<'a, Ctx>, T: 'static {
...
impl<'a> ContextArbitrary<'a, Context> for Expr {
    fn c_arbitrary(ctx: &mut Context, u: &mut Unstructured<'a>) -> Result<Self> {
        let enum_choices = [
            |ctx, u| Ok(ExprKind::Path(None, c_arbitrary(ctx, u)?))
        ];
        let choice = u.choose(&enum_choices)?;
...
impl<'a> ContextArbitrary<'a, Context> for Defaultness {
    fn c_arbitrary(ctx: &mut Context, u: &mut Unstructured<'a>) -> Result<Self> {
        let enum_choices = [|ctx|Defaultness::Default(span(ctx)), |_|Defaultness::Final];
        let choice = u.choose(&enum_choices)?;
...
impl<'a> ContextArbitrary<'a, Context> for Visibility {
    fn c_arbitrary(ctx: &mut Context, u: &mut Unstructured<'a>) -> Result<Self> {
        ...
        let choice = u.choose(&enum_choices)?;
        let kind = choice();
        Ok(Visibility { kind, span: span(ctx), tokens: None })
    }
}
...
impl<'a> ContextArbitrary<'a, Context> for Ident {
    fn c_arbitrary(_ctx: &mut Context, u: &mut Unstructured<'a>) -> Result<Self> {
        ...
        let name_token = with_default_session_globals(|| {
